...
class SamplerPubResult {
protected:
  std::unordered_map<std::string, BitArray>
      data_;       // in pair of creg name and bitstrings
  SamplerPub pub_; //
public:
  ...
  BitArray &data(void) { return data_[pub_.circuit().cregs()[0].name()]; }

  /// @brief Result data for the pub.
  /// @param name the name of the creg
  /// @return the bitarray for the creg name in the pub
  BitArray &data(const std::string &name) { return data_[name]; }
  ...
  /// @return pub
  const SamplerPub &pub(void) const { return pub_; }
